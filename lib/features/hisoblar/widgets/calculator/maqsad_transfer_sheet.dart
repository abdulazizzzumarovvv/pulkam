...
class MaqsadTransferSheet extends StatefulWidget {
  final MaqsadModel source; // qaysi maqsaddan o'tkaziladi
  final List<MaqsadModel> boshqalar; // qolgan maqsadlar (manzil)
  const MaqsadTransferSheet({
    super.key,
  ...
  @override
  State<MaqsadTransferSheet> createState() => _MaqsadTransferSheetState();
}

class _MaqsadTransferSheetState extends State<MaqsadTransferSheet>
    with SingleTickerProviderStateMixin {
  late final CalculatorCubit _calc;
  late final AnimationController _flash; // mablag' yetmasa sirena
  MaqsadModel? _selectedDest;
  ...
  void initState() {
    ...
    // O'tkaziladigan (manzil) maqsad default tanlangan
          ...
          return Container(
            ...
            child: Column(
              ...
              children: [
                ...
                // Manba maqsad (fixed) → strelka → manzil maqsadlar (tanlanadi)
                _sourceCard(),
  ...
  // ── Manba maqsad (fixed, sirena bilan) ────────────────────────────────
  Widget _sourceCard() {
  ...
  Widget _destPicker() {
    ...
    Widget item(MaqsadModel m) {
      final selected = _selectedDest?.key == m.key;
      return GestureDetector(
        onTap: () => setState(() => _selectedDest = m),
        child: _miniCard(
          color: m.color,
          name: m.name,
          balance: m.balance,
